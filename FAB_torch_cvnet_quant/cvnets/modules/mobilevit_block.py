...
from cvnets.ptq.layers import  QAct, QConv2d
...
class MobileViTBlock(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if(self.quant):
            ...
            act_bit = getattr(opts,"quant.activation_bit")
            act_bit_type = BIT_TYPE_DICT[act_bit]
        ...
        conv_3x3_in.block.act = Lin_Swish()
        if(self.quant):
            conv_3x3_in.block.conv = QConv2d(in_channels=conv_3x3_in.block.conv.in_channels,
                out_channels=conv_3x3_in.block.conv.out_channels,
                kernel_size=conv_3x3_in.block.conv.kernel_size,  
                stride=conv_3x3_in.block.conv.stride, 
                padding=conv_3x3_in.block.conv.padding,
                dilation=conv_3x3_in.block.conv.dilation,  
                groups=conv_3x3_in.block.conv.groups,
                bias=conv_3x3_in.block.conv.bias,
            )
            conv_3x3_in.block.act = Lin_Swish(bit_type = act_bit_type)
        ...
        if(self.quant):
            conv_1x1_in.block.conv = QConv2d(in_channels=conv_1x1_in.block.conv.in_channels,
                out_channels=conv_1x1_in.block.conv.out_channels,
                kernel_size=conv_1x1_in.block.conv.kernel_size,  
                stride=conv_1x1_in.block.conv.stride, 
                padding=conv_1x1_in.block.conv.padding,
                dilation=conv_1x1_in.block.conv.dilation,  
                groups=conv_1x1_in.block.conv.groups,
                bias=conv_1x1_in.block.conv.bias,
            )
        ...
        conv_1x1_out.block.act = Lin_Swish()
        if(self.quant):
            conv_1x1_out.block.conv = QConv2d(in_channels=conv_1x1_out.block.conv.in_channels,
                out_channels=conv_1x1_out.block.conv.out_channels,
                kernel_size=conv_1x1_out.block.conv.kernel_size,  
                stride=conv_1x1_out.block.conv.stride, 
                padding=conv_1x1_out.block.conv.padding,
                dilation=conv_1x1_out.block.conv.dilation,  
                groups=conv_1x1_out.block.conv.groups,
                bias=conv_1x1_out.block.conv.bias,
            )
            conv_1x1_out.block.act = Lin_Swish(bit_type = act_bit_type)
        ...
        if not no_fusion:
            ...
            conv_3x3_out.block.act = Lin_Swish()
            if(self.quant):
                conv_3x3_out.block.conv = QConv2d(in_channels=conv_3x3_out.block.conv.in_channels,
                    out_channels=conv_3x3_out.block.conv.out_channels,
                    kernel_size=conv_3x3_out.block.conv.kernel_size,  
                    stride=conv_3x3_out.block.conv.stride, 
                    padding=conv_3x3_out.block.conv.padding,
                    dilation=conv_3x3_out.block.conv.dilation,  
                    groups=conv_3x3_out.block.conv.groups,
                    bias=conv_3x3_out.block.conv.bias,
                )
                conv_3x3_out.block.act = Lin_Swish(bit_type = act_bit_type)
        ...
        if(self.quant):
            ...
            self.qact_norm = QAct(
                bit_type=act_bit_type,
                calibration_mode=calibration_a)
            # self.qact_proj_shift = QAct(
            #     bit_type=act_bit_type,
            #     calibration_mode=calibration_a)
            self.qact_concat = QAct(
                bit_type=act_bit_type,
                calibration_mode=calibration_a)
            # self.qact_fusion_shift = QAct(
            #     bit_type=act_bit_type,
            #     calibration_mode=calibration_a)
        ...
        # self.post_norm = get_normalization_layer(
        #         opts=opts,
    ...
    def forward_spatial(self, x: Tensor) -> Tensor:
        ...
        if(self.quant):
            res = self.qact_shortcut(x)
        else:
            res = x
        # conv 1x1
        if(self.quant):
            x = self.qact_kxk(x)
            #x = self.local_rep.conv_3x3(x)
            #scale,_ = self.qact_kxk.observer.get_quantization_params()
            scale = self.qact_kxk.quantizer.scale

            x = self.local_rep.conv_3x3.block.conv(x,scale)
            x = self.local_rep.conv_3x3.block.norm(x)
            x = self.local_rep.conv_3x3.block.act(x)
           

            x = self.qact_1x1(x)
            # conv 3x3
            fm = self.local_rep.conv_1x1(x)
            fm = self.qact_pwc(fm)
            # last_scale_factor , _ = self.qact_pwc.observer.get_quantization_params()
            last_scale_factor = self.qact_pwc.quantizer.scale
        else:
            fm = self.local_rep(x)
        ...
        for transformer_layer in self.global_rep:
            patches ,last_scale_factor = transformer_layer(patches,last_scale_factor=last_scale_factor)

        # [B x Patch x Patches x C] --> [B x C x Patches x Patch]
        # patches = patches.transpose(1,2)
        # patches = self.post_norm(patches)
        # if(self.quant):
        #     patches = self.qact_norm(patches)
        # patches = patches.transpose(1,2)
        
        fm = self.folding(patches=patches, info_dict=info_dict)
       
        if(self.quant):
            #scale , _= self.qact_norm.observer.get_quantization_params()
            fm = self.conv_proj.block.conv(fm,scale)
            fm = self.conv_proj.block.norm(fm)
            #fm = self.qact_proj_shift(fm)
            fm = self.conv_proj.block.act(fm)

            fm = self.qact_concat(torch.cat((res,fm),dim=1))
            # scale, _= self.qact_concat.observer.get_quantization_params()
            scale = self.qact_concat.quantizer.scale
            
            if self.fusion is not None:
                fm = self.fusion.block.conv(fm,scale)
                fm = self.fusion.block.norm(fm)
                fm = self.fusion.block.act(fm)
        else:
...
class MobileViTBlockv2(BaseModule):
    ...
    def __init__(
        ...
        conv_ksize: Optional[int] = 3,
        dilation: Optional[int] = 1,
        attn_norm_layer: Optional[str] = "batch_norm_2d",
        *args,
        **kwargs
    ) -> None:
    ...
    def forward_spatial(self, x: Tensor, *args, **kwargs) -> Tensor:
        ...
        fm = self.local_rep(x) ##conv 3x3   / conv 1x1
