...
class BaseImageEncoder(BaseAnyNNModel):
    ...
    def __init__(self, opts: argparse.Namespace, *args, **kwargs) -> None:
        ...
        if(self.quant):
            ...
            act_bit_type = BIT_TYPE_DICT[act_bit_type]
    ...
    def extract_features(self, x: Tensor, *args, **kwargs) -> Tensor:
        ...
        if(self.quant):
            x = self.qact_input(x)
            scale = self.qact_input.quantizer.scale
            x = self.conv_1.block.conv(x,scale)
            x = self.conv_1.block.norm(x)
            x = self.conv_1.block.act(x)
        else: 
        ...
        if(self.quant):
            x = self.qact_final_conv(x)
            scale = self.qact_final_conv.quantizer.scale
            x = self.conv_1x1_exp.block.conv(x,scale)
            x = self.conv_1x1_exp.block.norm(x)
            x = self.conv_1x1_exp.block.act(x)
        else:
            x = self._forward_layer(self.conv_1x1_exp, x)
    ...
    def forward_classifier(self, x: Tensor, *args, **kwargs) -> Tensor:
        ...
        if(self.quant):
            x = self.classifier.global_pool(x)
            x = self.qact_final(x)
            scale = self.qact_final.quantizer.scale
            x = self.classifier.classifier_fc(x,scale)
        else: 
            x = self.classifier(x)
        return x
