...
from cvnets.ptq.layers import  QAct, QConv2d, QLinear
...
class MobileViT(BaseImageEncoder):
    ...
    def __init__(self, opts, *args, **kwargs) -> None:
        ...
        self.conv_1.block.act = Lin_Swish()
        if(self.quant):
            self.conv_1.block.act = Lin_Swish(bit_type = act_bit_type,calibration_mode=calibration_c)
            # print(self.conv_1.block.act)
        ...
        self.conv_1x1_exp.block.act = Lin_Swish()
        if(self.quant):
            self.conv_1x1_exp.block.act=Lin_Swish(bit_type=act_bit_type)
    ...
    def open_calibration(self):
        for m in self.modules():
             if type(m) in [QConv2d,QLinear,QAct,Lin_Swish]:
                m.calibrate=True
