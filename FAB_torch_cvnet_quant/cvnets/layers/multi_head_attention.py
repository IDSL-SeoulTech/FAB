...
class MultiHeadAttention(BaseLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.qkv_proj = QLinear(in_features=embed_dim, out_features=3 * embed_dim, bias=bias)
       
        self.attn_dropout = Dropout(p=attn_dropout)
        self.out_proj = QLinear(in_features=embed_dim, out_features=output_dim, bias=bias)
    ...
    def forward_tracing(
        ...
        if x_kv is None:
            ...
            query =self.qact_q(query)
            key =self.qact_k(key)
            value =self.qact_v(value)
        else:
            # [N, S, C]
            query = F.linear(
                x_q,
        ...
        wt_out = self.qact_2(wt_out)
        wt_out = self.out_proj(wt_out)
        wt_out = self.qact_3(wt_out)
        return wt_out
    ...
    def forward_default(
        ...
        if x_kv is None:
            ...
            qkv = self.qkv_proj(x_q,last_scale_factor)
            if(self.quant):
                qkv = self.qact_qkv(qkv)
            qkv = qkv.reshape(b_sz, S_len, 3, self.num_heads, -1)
            # [N, S, 3, h, c] --> [N, h, 3, S, C]
        ...
        softmax_scale=None
        if(self.quant):
            if(self.qact_attn1.quant==True):
                # softmax_scale , _  = self.qact_attn1.observer.get_quantization_params()
                softmax_scale = self.qact_attn1.quantizer.scale
        attn_as_float = self.linmax(attn.float(), softmax_scale)

        #attn_as_float = self.softmax(attn.float())
        attn = attn_as_float.to(attn_dtype)
        attn = self.attn_dropout(attn)
        #attn = self.qact_softmax_output(attn)
        # weighted sum
        ...
        scale = None
        if(self.quant):
            if(self.qact_2.quant==True):
                # scale, _ = self.qact_2.observer.get_quantization_params()
                scale = self.qact_2.quantizer.scale
        out = self.out_proj(out,scale)
        #out = self.qact_3(out)
